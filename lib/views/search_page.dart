...
class StateSearchPage extends State<SearchPage> {
  ...
  onPressedOpenAndCloseTrip(index) {
    setState(() {
      tripList[index].isOpen == true
          ? tripList[index].isOpen = false
          : tripList[index].isOpen = true;
    });
  }
  ...
  Future<void> handleBooking(int index) async {
    bool isLoggedIn = await Config.checkLogin();
    ...
    if (isLoggedIn) {
      if (tripList[index].isBooking == false) {
        ...
        CustomAlertDailog.CustomActionDialog(
          context: context,
          titelText: " هل تريد الغاء الحجز؟",
          textButton1: "نعم",
          textButton2: "لا",
          onClick1: () {
            Navigator.of(context).pop();
            context.read<BookingPageBloc>()
              ..add(CancleBookingTripEvent(tripList[index].id, index));
          },
          onClick2: () {
            Navigator.of(context).pop();
          },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: MultiBlocProvider(
        providers: [
          BlocProvider(
              create: (context) => SearchBloc()..add(LoadSearchAPIEvent(false))),
                  ...
                  if (state is CancelBookingTripLoaded) {
                    ...
                    if (state.responseCode == 200) {
                      context.read<SearchBloc>()
                        ..add(UpdateTripEvent(tripList[state.index]));
                      if (Config.isShowingLoadingDialog == false) {
                        Config.isShowingLoadingDialog = true;
                        CustomAlertDailog.CustomLoadingDialog(
                            context: context,
                    ...
                    children: [
                      Container(
                        ...
                        child: CustomNavBar.customNaveBar(
                            context: context, title: "نتائج البحث"),
                      ),
                      CustomSectionComponent.Section(
                          context: context,
                          text:
                          "${textSearch == "" ? "غير معروف" : textSearch.length > 40 ? textSearch.substring(0, 40) : textSearch}",
                      ...
                      Container(
                        ...
                        child: tripList.isNotEmpty
                            ...
                            child: Column(
                              ...
                              children: [
                                SvgPicture.asset(isLoadedError
