            ...
            listener: (context,state){
              ...
              if(state is DonationRequestApiLoaded){
                ...
                if(state.responseCode == 200){
                  ...
                  var description = "";
                  if(isTrip){
                    description = "تم التبرع للرحلة بقيمة " +amountText.text+" شيقل";
                  } else {
                    description = "تم التبرع للمشروع بقيمة " +amountText.text +" شيقل";
                  }
              ...
              if(state is PaypalPaymentState){
                ...
                CustomAlertDailog.PayPalShowModalBottomSheet(context: context,checkoutUrl:state.checkoutUrl,returnURL:state.returnURL,cancelURL: state.cancelURL,action: (){
                  ...
                  context.read<DonationPageBloc>().add(DonationPageRequestApiEvent(isTrip==true?trip.id:project.id, isTrip?2:1,"", amountText.text, user != null? user.name:"unkown",numberOfPeople.text,nameCard.text,numberCard.text,expiryDate.text,cvv.text));
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            CustomSectionComponent.Section(context: context,text: "موجز التبرع",margin: EdgeInsets.only(top: 18,left: 16,right: 16),seeMore: false),


                            Container(
                              margin: EdgeInsets.only(left: 16,right: 16,top: 16),
                              child: Text("نوع التبرع: " + "تبرع " + sector.toString(),style: TextStyle(fontSize: 16,fontWeight: FontWeight.w600,fontFamily: 'SansArabicLight',color:Color(0xff101426))),
                            ),
                            ...
                            if(isTrip) Container(
                              child: CustomTextField.TextFieldWithTitleAndBorder(controller: numberOfPeople,width: MediaQuery.of(context).size.width-50,margin: EdgeInsets.only(left: 16,right: 16,top: 16),title: "عدد الاشخاص المتبرع لهم",hintText: "عدد الاشخاص المتبرع لهم",obscureText: false,textAlign: TextAlign.start,keyboardType: TextInputType.number),
                            ),
                                ...
                                children: [
                                      ...
                                      onPressed: (){
                                        ...
                                        CustomAlertDailog.CustomShowModalBottomSheet(context: context,nameCard: nameCard,numberCard: numberCard,expiryDate: expiryDate,cvv: cvv,onPressedCompleated: (singingCharacter){
                                          ...
                                          if(singingCharacter == SingingCharacter.paybal){
                                            context.read<DonationPageBloc>().add(PaypalPaymentEvent(isTrip==true?trip.nameProject:project.nameProject,amountText.text,numberOfPeople.text));
                                            Navigator.of(context).pop();
                                          } else {
                                            if(nameCard.text.isNotEmpty && numberCard.text.isNotEmpty && expiryDate.text.isNotEmpty && cvv.text.isNotEmpty) {
                                              context.read<DonationPageBloc>()..add(DonationPageRequestApiEvent(isTrip==true?trip.id:project.id, isTrip?2:1,"", amountText.text, user != null? user.name:"unkown","",nameCard.text,numberCard.text,expiryDate.text,cvv.text));
                                            } else {
                                              if( Config.isShowingLoadingDialog == false) {
                                                Config.isShowingLoadingDialog = true;
                                  ...
                                  Container(
                                    ...
                                    child: MaterialButton(
                                      ...
                                      child: Text("الغاء",style: TextStyle(fontSize: 14,fontWeight: FontWeight.w600,fontFamily: 'SansArabicLight',color:Theme.of(context).primaryColor,height: 1.4),),
                                      onPressed: (){
                                        Navigator.of(context).pop();
                                      },
                                    ),
