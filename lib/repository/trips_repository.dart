...
class TripsRepository {
  ...
  getTrips(api,skip,token) async{
    ...
    try {
      ...
      if(token != "" && token.toString() != "null"){
        ...
        response = await http.get(Uri.parse(api+"?skip=${skip}&lat=${latLng.lat}&lng=${latLng.lng}"));
      }
  ...
  searchTrips(api,search,token) async{
    ...
    try {
      ...
      var response;
      if(token != "" && token.toString() != "null"){
        response = await http.get(Uri.parse(api+"?search=${search}&lat=${latLng.lat}&lng=${latLng.lng}"),headers: {"Authorization":"Bearer "+token.toString()});
      } else{
        response = await http.get(Uri.parse(api+"?search=${search}&lat=${latLng.lat}&lng=${latLng.lng}"));
      }
